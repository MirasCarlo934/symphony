...
function msg(ito) {
	alert(ito.id+ "\n" +ito.innerHTML);
//	var rule  = rulesForEdit[ito.id.split("_")[0]];
 ...
 */
function init() {
	var d_timer = new Device("TIME", "Timer", "Timer", "U7YY", [{id:"0001",label:"Active",io:"O",min:0,max:1},{id:"0002",label:"Duration(Mins)",io:"I",min:0,max:240}]);
	deviceList[deviceList.length] = d_timer;	
...
str = str + "\narg j:"+j+" arg.id:"+args[j].id;		
			...
			var props = args[j].properties;
			for (var i = 0; i < props.length; i++) {
				var divId = rule.id + "_arg_" + args[j].id + "_" + props[i].id;
str = str + "\n          rule.id:"+rule.id+" props.i:"+i+" id:"+divId +" p.id:"+props[i].id+" "+props[i].operator+" "+props[i].value;
				...
				var btn = document.getElementById(divId+"_1");
				markProp(btn, true);
			}
			rules[k].id = rule.id;
			var theDevice = document.getElementById(rule.id + "_arg_" + args[j].id);
...
str = str + "\nexec j:"+j+" id:"+exec[j].id;
			...
			var props = exec[j].properties;
			for (var i = 0; i < props.length; i++) {
				var divId = rule.id + "_ctrl_" + exec[j].id + "_" + props[i].id;
str = str + "\n          props.i:"+i+" id:"+divId +" p.id:"+props[i].id+" "+props[i].operator+" "+props[i].value;
			...
			var theDevice = document.getElementById(rule.id + "_ctrl_" + exec[j].id);
			unhoverDevice(theDevice);	//displays the selected properties
		}
str = str + "\nrule.name:"+rules[k].name+" id:"+rules[k].id+" args:"+args.length+"\n";
		toggleRule(rules[k].id+"_header");  //we display the rules in minimized view
	}
	str = str +"\n</rules>";
//	alert(str)
	var ta = document.getElementById("ta");
...
function drawRule(data, count, menu) {
	...
	if (dropped!=null) {
		...
		if(eRule==null) {
			...
			$("#"+rule.id+"_header_txt").val(rules[count-1].name);//we are renaming the rule
...
//alert("ito.id:"+ito.id+" arrangeProps label:"+props[i].label+" top:"+p.style.top+" included:"+p.included+"\nito.id:"+ito.id+" p.id:"+p.id+" p.label:"+document.getElementById(p.id+"_lbl").innerHTML+" top:"+p.style.top+" included:"+p.included);
			row++;
		}
	}
	for(var i = 0; i < props.length; i++){
...
function unhoverDevice(ito) {
		...
		for(var i = 0; i < prop.length; i++){
			...
			if (p.included==true) {
				...
				row++;
//alert("ito.id:"+ito.id+" p.id:"+p.id+" p.label:"+document.getElementById(p.id+"_lbl").innerHTML+" top:"+p.style.top+" included:"+p.included);
			} else {
				//p.setAttribute("style","display:none");
				var rect = ito.getBoundingClientRect();
//alert("before ito.id:"+ito.id+" d.h:"+device.height+" r.h:"+rect.height+" p.id:"+p.id+" p.label:"+document.getElementById(p.id+"_lbl").innerHTML+" top:"+p.style.top+" included:"+p.included);
//				p.style.top = rect.height+"px";
				p.style.top = (device.height+2)+"px";				
//alert("after ito.id:"+ito.id+" p.id:"+p.id+" p.label:"+document.getElementById(p.id+"_lbl").innerHTML+" top:"+p.style.top+" included:"+p.included);
			}
		}		
...
The Device object
  ...
  properties is an array of properties for this device
  
*/
function Device(id, type, name, room, properties) {
  this.id = id;
  this.type = type; 
  this.name = name;
  this.room = room;
...
//        	p.innerHTML="<center>"+name+"</center>";
        	...
        	tdBtnRemove.setAttribute('id', p.id+"_remove");
  ...
  this.addProperty = function(index, prop) {
	    var div=document.getElementById(this.id);
	    var p = document.getElementById(this.id+"_"+prop.id);
	    if (p==null) {
...
//  this.deviceCtrls = new Object();
  ...
  this.create = function() {
	...
	var p = document.createElement('p');
	p.setAttribute('class', "pHeader");
	p.setAttribute('id', this.id+"_header");
	...
	tdLbl.innerHTML="<input type='text' style='width:100%;text-align: center; background-color: lightgray;	border: none;color: black;font-weight: bold;font-family:Verdana, Arial, Helvetica, sans-serif;' value='"+name+"' id='"+p.id+"_txt' disabled>";
	
	var tdEdit = document.createElement('td');
	tdEdit.setAttribute('id', p.id+"_edit");
	...
	tdBtnMin.setAttribute('id', p.id+"_min");
...
function  deviceGetter() {
	...
	for(var i = 0; i < this.properties.length; i++) {
		if (this.properties[i].included == true) {
			var ruleId = this.id.split("_")[0];
			var deviceId = this.id.split("_")[2];
...
function selectRuleObj(ito) {
	...
	if (selectedRule.length!=0) {
		if($("#"+selectedRule).length) {//test if the selectedRule still exists, it might have already been removed
			...
			document.getElementById(selectedRule+"_header").removeAttribute("style");
...
//	ito.setAttribute("style","background-color:darkgray;color:white");
	...
	document.getElementById(ruleId[0]+"_header").style.border="1px solid green";
...
function editRule(editBtn){
	var str = editBtn.id.replace("_edit", "_txt");
	...
	eRule.addEventListener('blur', function (evt) {
		...
		rulesForEdit[eRule.id.split("_")[0]].name = $("#"+str).val();
    });
}
/*
...
function toggleRule(ruleIdHeader){
	var str = ruleIdHeader.replace("_header", "");
	if ($("#"+str+"_arg").is(':visible')) {
		//the _arg element is visible
		$("#"+str).animate({height : $("#"+ruleIdHeader).outerHeight()+"px", width: "250px"}); 
		$("#"+str+"_arg").hide(400)
		$("#"+str+"_ctrl").hide(400)
		$("#"+ruleIdHeader+"_min").html('<img class="pHover minmax" src="/pics/max.png"/>');	
	} else {
		...
		$("#"+ruleIdHeader+"_min").html('<img class="pHover minmax" src="/pics/min.png"/>');
	}
}
/*
...
function toggleDrag(ito, isEnabled) {
	...
	var containerId = eId[0] +"_"+ eId[1] + "_"+ eId[2];
	var container = document.getElementById(containerId)
...
function showDroppedDevice(tCategory, category, i, rule, evt, rectTarget, isNewRule, isCIR) {
	...
	if (tCategory == "CMPSR" || (tCategory == "CTRL" && category=="ctrlMenuItem") || (tCategory == "ARG" && category=="snsrMenuItem")){
		var deviceIdLocal = rule.id+str+deviceList[i].id;
		var deviceName = deviceList[i].name;
		if (deviceList[i].id == "TIME") { //we are creating a new virtual Timer instance 
			//override the deviceId and deviceName above
			deviceIdLocal = rule.id+str+deviceList[i].id + timerCount;
			deviceName = deviceList[i].name + timerCount;
			timerCount++;
		}
		var device = new Device(deviceIdLocal,"type",deviceName,deviceList[i].room, deviceList[i].properties);
...
function showTransferredDevice(tCategory, category, i, rule, fromDevice) {
	...
	var device = new Device(rule.id+str+fromDevice.id.split("_")[2],fromDevice.type,fromDevice.name,fromDevice.room,fromDevice.properties);
	...
	for(var i = 0; i < props.length; i++){
		...
		var p = document.getElementById(device.id+"_"+props[i].id);
		var frP = document.getElementById(fromDevice.id+"_"+frProps[i].id);
		p.included = frProps[i].included;
		p.style.top = frP.style.top;
		var btn = document.getElementById(device.id+"_"+props[i].id+"_1");
		var frBtn = document.getElementById(fromDevice.id+"_"+frProps[i].id+"_1");
		btn.setAttribute("style",frBtn.getAttribute("style"));
		var input = document.getElementById(device.id+"_"+props[i].id+"_input");
		var frInput = document.getElementById(fromDevice.id+"_"+frProps[i].id+"_input");
		input.value = frInput.value;
		var tdOpt = document.getElementById(device.id+"_"+props[i].id+"_cond");
...
function transferDevice(dropped, target) {
	if (dropped.id.split("_")[0]!=target.id.split("_")[0]) {
		if (dropped.id.split("_")[1]==target.id.split("_")[1]){
			var fromId = dropped.id.split("_");
			var ruleFrom = rulesForEdit[fromId[0]];
			var device = ruleFrom.devices[dropped.id];
			//we will add the device to the target rule
			var ruleTo = rulesForEdit[target.id.split("_")[0]];
...
function compose() {
	...
	var ta = document.getElementById("ta");
	ta.value = str;
	var taData = $("#taForm").serialize();
	$.get("/devices/composeCIR", taData, function(data, status) {
		$("#notif").html(data);
	});
...
//		  alert("readystate:" + this.readyState+ " status:"+this.status);
//	    if (this.readyState == 4 && this.status == 200) {
//	    	var txt = document.getElementById("notif");
//	        txt.innerHTML=xhttp.responseText;
//	    }
//	  };
//	  xhttp.open("POST", "/devices/composeCIR", true);
//	  xhttp.send("cir="+str);
}
