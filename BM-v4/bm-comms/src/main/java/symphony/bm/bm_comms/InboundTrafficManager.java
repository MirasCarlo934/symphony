...
public class InboundTrafficManager {
...
//        t.start();
    ...
    public void addInboundRawMessage(RawMessage rawMsg) {
        executor.submit(new ProcessingUnit(rawMsg));
    }

    private class ProcessingUnit implements Runnable {
        private RawMessage rawMsg;

        private ProcessingUnit(RawMessage rawMsg) {
            this.rawMsg = rawMsg;
        }
        ...
        private boolean checkIfDeviceExists(String cid) {
            Document device = mongoDBManager.getDevicesCollection().find(eq("CID", cid)).first();
            return (device != null);
        }
