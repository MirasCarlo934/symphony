...
void FillLEDsFromPaletteColors( uint8_t colorIndex, uint8_t brightness, uint8_t increment)
{
    ...
    for( int i = 0; i < NUM_PIXELS; i++) {
    	for(int j=0; j<LED_STRINGS;j++) {
//    		leds[j][i] = ColorFromPalette( currentPalette1, colorIndex, brightness, currentBlending);
    		theLeds[j][i] = ColorFromPalette( palettes[paletteIndex].palette, colorIndex, brightness, currentBlending);
    	}
//        colorIndex += 3;
        colorIndex += increment;
...
void FillLEDsAsSolid()
{
	...
	for( int i = 0; i < NUM_PIXELS; i++) {
		for(int j=0; j<LED_STRINGS;j++) {
			theLeds[j][i] = ColorFromPalette( currentPalette1, colorIndex, brightness, LINEARBLEND);
//        		leds[j][i] = ColorFromPalette( currentPalette1, colorIndex, brightness, NOBLEND);
		}
		colorIndex += 3;
	}*/
 ...
 * Creates the pixel colors from the palette.
 */
void FillLEDsAsStatic(int colorIndex)
{
    uint8_t brightness = 255;
	...
	for (int i=0; i<pixelCount;i++) {
		for (int j=0; j<stringCount;j++){
			theLeds[j][i] = ColorFromPalette( palette, colorIndex, brightness, NOBLEND);
			colorIndex += 3;
		}
	}
    /*for( int i = 0; i < NUM_PIXELS; i++) {
...
CRGBPalette16 SetupTotallyRandomPalette()
{
	CRGBPalette16 plt;
    for( int i = 0; i < 16; i++) {
        plt[i] = CHSV( random8(), 255, random8());
    }
    return plt;
}
...
{
    fill_solid( currentPalette1, 16, CRGB::Green);
    // and set every fourth one to Red.
    currentPalette1[0] = CRGB::Red;
    currentPalette1[4] = CRGB::Red;
    currentPalette1[8] = CRGB::Red;
    currentPalette1[12] = CRGB::Red;
